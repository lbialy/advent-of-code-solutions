...
trait Task6a extends AdventOfCodeTask {
  ...
  final case class TurnOn(sx: Int, sy: Int, ex: Int, ey: Int) extends Instruction
  final case class TurnOff(sx: Int, sy: Int, ex: Int, ey: Int) extends Instruction
  final case class Toggle(sx: Int, sy: Int, ex: Int, ey: Int) extends Instruction

  val regex = """(\d\d?\d?,\d\d?\d?)""".r

  def parseInstruction(str: String): Instruction = {
    val coords = regex.findAllIn(str).toList
    val startCoord = coords.head.split(",").map(_.toInt)
    val endCoord = coords.tail.head.split(",").map(_.toInt)

    str match {
      case x if x startsWith "turn on" => TurnOn(startCoord.head, startCoord.tail.head, endCoord.head, endCoord.tail.head)
      case x if x startsWith "turn off" => TurnOff(startCoord.head, startCoord.tail.head, endCoord.head, endCoord.tail.head)
      case x if x startsWith "toggle" => Toggle(startCoord.head, startCoord.tail.head, endCoord.head, endCoord.tail.head)
    }
  }

  def applyInstruction(tbl: ChristmasLightsMatrix, ins: Instruction): Unit = {
    for (i <- ins.sx to ins.ex) {
      for (j <- ins.sy to ins.ey) {
        ins match {
          case _: Toggle => tbl(i)(j) = if (tbl(i)(j) == 0) 1 else 0
          case _: TurnOff => tbl(i)(j) = 0
          case _: TurnOn => tbl(i)(j) = 1
        }
      }
    }
  }

  def countLitLights(tbl: ChristmasLightsMatrix) = tbl.foldLeft(0)(_ + _.foldLeft(0)(_ + _)) // don't use sum for Byte
}
